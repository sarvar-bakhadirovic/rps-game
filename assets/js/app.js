...
const _scoreNull = document.querySelector('.header__score')

// LocalStorage
let score = JSON.parse(localStorage.getItem('score')) ? (0 + JSON.parse(localStorage.getItem('score'))) : 0
...
_buttons.forEach(btn => {
    btn.addEventListener('click', e => {
        ...
        if (icon.classList.contains('rock-icon')) {
            _participant.style.cssText = `border: 30px solid #dd405d; background-color: #fff;`
        } else if (icon.classList.contains('paper-icon')) {
            _participant.style.cssText = `border: 30px solid #5671f5; background-color: #fff;`
        } else if (icon.classList.contains('scissors-icon')) {
            _participant.style.cssText = `border: 30px solid #eca922; background-color: #fff;`
        }
...
function randomElement() {
    ...
    if (num == 1) {
        ...
        _computer.style.cssText = `border: 30px solid #dd405d; background-color: #fff;`
    } else if (num == 2) {
        _computer.innerHTML = paper
        _computer.style.cssText = `border: 30px solid #5671f5; background-color: #fff;`
    } else if (num == 3) {
        _computer.innerHTML = scissors
        _computer.style.cssText = `border: 30px solid #eca922; background-color: #fff;`
    }
}
